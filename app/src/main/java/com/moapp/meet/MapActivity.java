...
import com.moapp.meet.model.MapItem;
...
public class MapActivity extends AppCompatActivity implements MapView.CurrentLocationEventListener, MapReverseGeoCoder.ReverseGeoCodingResultListener, MapView.POIItemEventListener{
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        ...
                        if (task.isSuccessful()) {
                            ...
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                ...
                                getMarker(document.getString("SearchTitle"), document.getString("SearchLat"), document.getString("SearchLng"),document.getString("category"));
                                m_arr.add(new MapItem(document.getId(),document.getString("SearchTitle"), document.getString("SearchLat"), document.getString("SearchLng"), document.getString("category")));
    ...
    private MapPoint getMarker(String title,String latitude, String longitude,String category) {
        int tagnum = 10;
        ...
        if(category.equals("sports")){
            marker.setCustomImageResourceId(R.drawable.map_sports); // 마커 이미지.
            marker.setCustomImageAutoscale(false); // hdpi, xhdpi 등 안드로이드 플랫폼의 스케일을 사용할 경우 지도 라이브러리의 스케일 기능을 꺼줌.
            marker.setCustomImageAnchor(0.5f, 1.0f);
        }else if(category.equals("study")){
            marker.setCustomImageResourceId(R.drawable.map_study); // 마커 이미지.
            marker.setCustomImageAutoscale(false); // hdpi, xhdpi 등 안드로이드 플랫폼의 스케일을 사용할 경우 지도 라이브러리의 스케일 기능을 꺼줌.
            marker.setCustomImageAnchor(0.5f, 1.0f);
        }else if(category.equals("language")){
            marker.setCustomImageResourceId(R.drawable.map_language); // 마커 이미지.
            marker.setCustomImageAutoscale(false); // hdpi, xhdpi 등 안드로이드 플랫폼의 스케일을 사용할 경우 지도 라이브러리의 스케일 기능을 꺼줌.
            marker.setCustomImageAnchor(0.5f, 1.0f);
        }
        else if(category.equals("friend")){
            marker.setCustomImageResourceId(R.drawable.map_friend); // 마커 이미지.
            marker.setCustomImageAutoscale(false); // hdpi, xhdpi 등 안드로이드 플랫폼의 스케일을 사용할 경우 지도 라이브러리의 스케일 기능을 꺼줌.
            marker.setCustomImageAnchor(0.5f, 1.0f);
        }
        else if(category.equals("book")){
            marker.setCustomImageResourceId(R.drawable.map_book); // 마커 이미지.
            marker.setCustomImageAutoscale(false); // hdpi, xhdpi 등 안드로이드 플랫폼의 스케일을 사용할 경우 지도 라이브러리의 스케일 기능을 꺼줌.
            marker.setCustomImageAnchor(0.5f, 1.0f);
        }
        else if(category.equals("etc")){
            marker.setCustomImageResourceId(R.drawable.map_etc); // 마커 이미지.
            marker.setCustomImageAutoscale(false); // hdpi, xhdpi 등 안드로이드 플랫폼의 스케일을 사용할 경우 지도 라이브러리의 스케일 기능을 꺼줌.
            marker.setCustomImageAnchor(0.5f, 1.0f);
        }
                ...
                if (index == 0) {
                    for(int i=0; i<m_arr.size(); i++){
                        if(location_name == m_arr.get(i).AddressName){
                            DocumentReference docRef = db.collection("board"+m_arr.get(i).Category).document(m_arr.get(i).getId());
                            docRef.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                                ...
                                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                                    if (task.isSuccessful()) {
                                        DocumentSnapshot document = task.getResult();
                                        if (document.exists()) {
                                            search=new BoardListItem(document.getId(),document.getString("ttitle"), document.getString("tname"), document.getLong("tdate"),  document.getString("tcontent"), document.getString("tlocation"),document.getString("time"));
                                            ...
                                            intent.putExtra("LOCATION",search.Location);
                                            intent.putExtra("TIME",search.Time);
